...
```bash
git clone git@github.com:youragency/wp-local-manager.git ~/wp-local-manager
cd ~/wp-local-manager
cp .env.example .env
```
...
```bash
./scripts/swap-site.sh pfverdun
```

**Get latest files from server:**
```bash
./scripts/sync-site.sh pfverdun
```

**Get latest files + fresh DB:**
```bash
./scripts/sync-site.sh pfverdun --db
./scripts/reimport-db.sh pfverdun
```
...
```bash
./scripts/sync-site.sh pfverdun --force
```
...
```bash
cd ~/wp-sites/pfverdun/wp-content/themes/your-theme
...
~/wp-local-manager/          ← this repo
  ...
  sites.json                 ← all site configs (committed to git)
...
2. They follow steps 1–12 above (their own SSH keys)
...
6. `sites.json` already has all configs — they run `setup-site.sh` per site as needed
